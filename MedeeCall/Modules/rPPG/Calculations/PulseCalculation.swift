...
class PulseCalculation {
    ...
    let maxDelta = 100.0 // Max RGB delta (empirical)
    ...
    func calculateHeartRate( _ actualFPS:Double){
        ...
        if( useConstRGBData == 1){
            ...
            frameRate = Double(VideoSettings.getFrameRate())
            (rawRedPixels, rawGreenPixels, rawBluePixels) = rgbSampleData.getRGBData()
        }else if( useConstRGBData == 2){
            let fps = VideoSettings.getFrameRate()
            let count = VideoSettings.getFramesPerHeartRateSample()
...
//        normalizedBlueAmplitude = normalizePixels( deltaRawBlue! )
        ...
        let filterStart = VideoSettings.getFilterStart()
        let filterEnd = VideoSettings.getFilterEnd()
